  ...
  const items = [
    {
      ...
      icon: <MdEngineering className="text-[#1F5A8B] text-5xl" />,
    },
    {
      title: 'Vision',
      description:
      ...
      icon: <MdOutlineTrendingUp className="text-[#1F5A8B] text-5xl" />,
    },
    {
      title: 'Values',
      description:
        'Integrity, Excellence, Innovation, Customer Satisfaction, and Commitment to Quality in every product we deliver.',
      icon: <MdOutlineVerified className="text-[#1F5A8B] text-5xl" />,
    },
  ]

  return (
    <section className="relative bg-[#2E2E2E] py-20 px-4 lg:px-16 overflow-hidden">
      

      <div className="mx-auto text-center mb-16">

        <h2 className="text-2xl md:text-4xl font-extrabold text-[#F5F5F5] mb-4">
      ...
      <div className="flex flex-col md:flex-row items-stretch gap-4 lg:gap-8 max-w-6xl mx-auto">
        {items.map((item, idx) => (
          <motion.div
            key={idx}
            className="flex-1 rounded-3xl p-6 text-center bg-white shadow-lg hover:shadow-2xl transform transition hover:-translate-y-2"
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.3 }}
            transition={{ duration: 0.6, delay: idx * 0.2 }}
          >
            <div className="flex justify-center text-amber-500 mb-6">{item.icon}</div>
